...
import { ArrowRight, Check, Eye, EyeOff, WalletCards } from 'lucide-react';
  ...
  return (
    <main className="min-h-screen bg-[var(--bg-primary)] px-5 py-8 text-[var(--text-main)] sm:grid sm:place-items-center">
      <section className="mx-auto w-full max-w-[700px]">
        ...
        <div className="grid overflow-hidden rounded-[1.75rem] border border-[var(--border-subtle)] bg-[var(--bg-surface)] shadow-[0_20px_60px_-34px_rgba(27,32,44,.45)] md:grid-cols-[.8fr_1.2fr]">
          {/* Left decorative panel */}
          <aside className="bg-[var(--warm-surface)] p-7 md:p-9">
            <p className="text-sm font-medium text-[var(--accent)]">Start with a clean slate</p>
            <h1 className="mt-3 text-3xl font-semibold tracking-[-.04em]">Make space for clarity.</h1>
            <ul className="mt-7 space-y-3 text-sm leading-6 text-[var(--text-muted)]">
              {[
                'No imported demo balances',
                'Your data stays separated by account',
                'Amounts are stored with exact minor-unit precision',
              ].map((item) => (
                <li className="flex gap-2" key={item}>
                  <Check className="mt-1 h-4 w-4 shrink-0 text-[var(--income)]" />
                  {item}
                </li>
              ))}
            </ul>
          ...
          <div className="p-7 md:p-9">
            ...
            <p className="mt-1 text-sm text-[var(--text-muted)]">It takes less than a minute.</p>
            ...
            <form className="mt-6 space-y-3.5" onSubmit={submit} noValidate>
              ...
              <label className="block text-sm font-medium">
                ...
                <span id="password-help" className="mt-1 block text-xs font-normal text-[var(--text-muted)]">
